...
void UOMAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetCurrentRageAttribute())
	{
		...
		{
			UOMFunctionLibrary::AddGameplayTagToActorIfNone(Data.Target.GetAvatarActor(), 
				OMGameplayTags::Player_Status_Rage_Full);
		}
		else if (GetCurrentRage() == 0.0f)
		{
			UOMFunctionLibrary::AddGameplayTagToActorIfNone(Data.Target.GetAvatarActor(),
				OMGameplayTags::Player_Status_Rage_None);
		}
		else
		{
			UOMFunctionLibrary::RemoveGameplayTagFromActorIfFound(Data.Target.GetAvatarActor(),
				OMGameplayTags::Player_Status_Rage_Full);
			UOMFunctionLibrary::RemoveGameplayTagFromActorIfFound(Data.Target.GetAvatarActor(),
				OMGameplayTags::Player_Status_Rage_None);
		}
